...
- `AGENTS.md` concisely establishes one independently deliverable Work Contract per Issue, normally one final delivery PR per Issue, Issue-backed branch/worktree use, `Refs #N` linkage, reconstructible handoff evidence, green-verification merge gates, Main-CI-gated Issue completion, and human resolution for PRD/EDD changes;
...
- the finalizer uses the commit-associated Pull Request API, exact standalone `Refs #N` lines, real-open-Issue checks, and idempotent comments; only success closes with the `completed` state reason, every non-success conclusion comments without closing, no path deletes Issues, and PR CI, direct pushes, missing references, closed Issues, and Pull Requests masquerading through the Issues API safely exit;
